...
struct node *
create_node(enum node_type type, int operand, struct node *l, struct node *r)
{
...
struct node *build_par(int lp, int rp, int q, int p)
{
        ...
        if (lp == q)
                return parse_expr(p+1, p+1, prt, parse_expr(p+2, rp, NULL, NULL));
        if (rp == p)
                return parse_expr(q-1, q-1, parse_expr(lp, q-2, NULL, NULL), prt);
...
struct node *parse_single(int lp, struct node *ln, struct node *rn)
{
        switch (stream[lp].type) {
                ...
                case '+': return bop_single(ln, rn, PLUS);
                case '*': return bop_single(ln, rn, MUL);
                case '(':
                          error("Unmatched (");
                          return NULL;
                case ')':
